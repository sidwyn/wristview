...
def wrist_camera_offset(config: dict) -> np.ndarray:
    """The wrist camera's pose in the end-effector frame.

    A real wrist camera sits behind and above the jaws, tilted down so the
    grasp point is in view. Translation is metres, rotation is roll, pitch,
    yaw in degrees.
    """
    from scipy.spatial.transform import Rotation

    translation = np.asarray(config.get("translation_m", [0.0, -0.04, -0.08]), dtype=np.float64)
    rpy = np.asarray(config.get("rotation_rpy_deg", [0.0, -25.0, 0.0]), dtype=np.float64)
