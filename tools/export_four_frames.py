...
st=json.loads((run/"05_render/demo_0/status.json").read_text()); src=np.array(st["source_frame_index"])
...
n=np.array(plane["normal"]); d=float(plane["offset_m"])
op=np.load(run/"03_estimate/demo_0/object_pose.npy")
times=tr["timestamps_s"][src]; fps=float(tr["source_fps"])
...
out=S/"fourframes_ego"; out.mkdir(exist_ok=True)
tiles=[]
for lab,k in picks:
